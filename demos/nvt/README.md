...
---
## Dependencies
The code relies on the same JAX-based software stack as the main simulation framework:
- `jax==0.4.30`
- `jax-md==0.2.8`
- `flax==0.8.5`

The implementation has been tested on an NVIDIA A100 GPU (40 GB VRAM). Due to the large cutoff required for MBD interactions, the memory demands of JAX, and the inclusion of the trimmed-SchNet surrogate model, a GPU with substantial VRAM is strongly recommended for such simulations.

Please refer to the official [JAX](https://github.com/jax-ml/jax) GPU installation guide for hardware-specific setup instructions.

---
## Running the imulation
After installing the required dependencies and applying the JAX-MD modifications described above, the simulation can be launched directly via:
```
python nvt_full.py
```
Users should carefully verify the simulation settings before execution, including the PE melt system size, selected vdW model, temperature, integration timestep, total number of MD steps, trajectory sampling interval, and other runtime parameters.
